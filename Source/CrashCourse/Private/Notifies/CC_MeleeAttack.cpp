...
TArray<FHitResult> UCC_MeleeAttack::PerformSphereTrace(USkeletalMeshComponent* MeshComp) const
{
	...
	const FVector End = Start - SocketExtensionOffset;
	...
	bool const bHit = World->SweepMultiByChannel(OutHits, Start, End, FQuat::Identity, ECC_Visibility,
		FCollisionShape::MakeSphere(SphereTraceRadius), Params, ResponseParams);

	if (bDrawDebugs)
	{
		DrawDebugSphereTraceMulti(World, Start, End, SphereTraceRadius, EDrawDebugTrace::ForDuration, bHit,
			OutHits, FColor::Red, FColor::Green, 5.f);
	}
