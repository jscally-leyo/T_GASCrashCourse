	...
	const FGameplayEventData* TriggerEventData)
{
	...
	check(OwningEnemy.IsValid()); // Should never be a nullptr, but checking never hurts

	OwningAIController = Cast<AAIController>(OwningEnemy->GetController());
	check(OwningAIController.IsValid()); // Should never be a nullptr, but checking never hurts
...
void UCC_SearchForTarget::StartSearch()
{
	...
	{
		GEngine->AddOnScreenDebugMessage(-1, 3.f, FColor::Red, FString::Printf(TEXT("UCC_SearchForTarget: Searching for target")));
	}
...
{if
	(!OwningEnemy.IsValid()) return;
	const FVector SearchOrigin = GetAvatarActorFromActorInfo()->GetActorLocation();
	
	FClosestActorWithTagResult ClosestActorResult = UCC_BlueprintLibrary::FindClosestActorWithTag(GetAvatarActorFromActorInfo(),
		SearchOrigin,CrashTags::Player, OwningEnemy->SearchRange);
...
void UCC_SearchForTarget::MoveToTargetAndAttack()
{
	...
	MoveToLocationOrActorTask = UAITask_MoveTo::AIMoveTo( //.Get() is needed because those are weak object pointers
	...
	MoveToLocationOrActorTask->ConditionalPerformMove(); // = "activate"
}
...
void UCC_SearchForTarget::AttackTarget(TEnumAsByte<EPathFollowingResult::Type> Result, AAIController* AIController)
{
	...
	AttackDelayTask = UAbilityTask_WaitDelay::WaitDelay(this, OwningEnemy->GetTimelineLength()); // Check BP on enemy class for function implementation
